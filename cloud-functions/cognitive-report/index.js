...
exports.main = async (event) => {
  try {
    const { elderDeviceId, familyDeviceId, days = 7 } = event;
    ...
    const averageResponseTimeMs = totalQuestions > 0 ? Math.round(totalResponseTime / totalQuestions) : 0;
    ...
    if (logs.length >= 4) {
      ...
      const firstRate = firstHalf.filter((l) => l.isCorrect).length / firstHalf.length;
      const secondRate = secondHalf.filter((l) => l.isCorrect).length / secondHalf.length;
